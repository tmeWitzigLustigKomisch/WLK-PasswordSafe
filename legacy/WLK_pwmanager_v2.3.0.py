...
Deutsch:
Dieser Passwortmanager speichert Ihre Passwörter sicher in einer verschlüsselten Tresor‑Datei. Er
nutzt eine dreifache Verschlüsselungskaskade (AES‑256‑GCM, XOR‑Obfuskation mittels HMAC‑Pad und
ChaCha20‑Poly1305). Über den scrypt‑KDF werden drei unabhängige Schlüssel (AES‑, ChaCha‑ und
MAC‑Schlüssel) aus Ihrem Master‑Passwort abgeleitet. Ein HMAC‑SHA512 sichert die Integrität der
Daten. Bei jedem Speichervorgang werden Salt/Nonce/Pad neu generiert, sodass die Datei binär
immer anders aussieht. Das Speichern erfolgt atomar; optional können vor dem Überschreiben
Backups der vorherigen Version angelegt werden. Eine optionale Passwortstärkewarnung erinnert
an die Mindestlänge des Master‑Passworts.
...
English:
This password manager stores your passwords securely in an encrypted vault file. It uses a triple
encryption cascade (AES‑256‑GCM, XOR obfuscation via an HMAC pad and ChaCha20‑Poly1305). The
scrypt KDF derives three independent keys (AES, ChaCha and MAC keys) from your master password.
An HMAC‑SHA512 protects the data integrity. Each save operation regenerates salts, nonces and pads
so the file always looks different at the binary level. Saving is atomic and can optionally create
backups. An optional password strength warning reminds you of the minimum length of the master
password.
...
# Tresor an.
VERSION = 4  # Dateiformat-Version. 4 führt eine zusätzliche äußere Verschlüsselungsschicht ein
SALT_LEN = 16
NONCE_LEN = 12
...
#
# **Version 4:**
# ``[MAGIC(4)][VER=4(1)][KDF-TLV(var)][salt_extra(16)][nonce_extra(12)][xor(v3_blob)…][hmac2(64)]``
#  – Zunächst wird ein v3‑Blob wie oben erzeugt (MAGIC+3+TLV+…+hmac).  Dieser v3‑Blob wird dann
#    mit einem One‑Time‑Pad verschleiert, das aus dem Master‑Passwort und ``salt_extra``
#    abgeleitet wird.  Anschließend wird über den endgültigen Header, ``salt_extra``, ``nonce_extra``
#    und den verschleierten Blob eine weitere HMAC gebildet.  Bei der Entschlüsselung wird
#    zunächst die äußere HMAC verifiziert, das Pad entfernt und anschließend der v3‑Blob
#    wie gewohnt verarbeitet.
#
# Die ursprüngliche Schrittfolge der Triple‑Verschlüsselung bleibt unverändert:
# ``plaintext -> AES‑GCM -> XOR‑Pad -> ChaCha20‑Poly1305 -> HMAC``.  In Version 4 folgt
# zusätzlich ``-> XOR‑Pad2 -> HMAC2``.
...
def encrypt_vault_bytes(plaintext: bytes, master_pw: bytes) -> bytes:
    ...
    In Version 4 des Dateiformats wird die bisherige Triple‑Verschlüsselung (AES‑GCM → XOR‑Pad →
    ChaCha20‑Poly1305 → HMAC) um eine zusätzliche äußere Schicht erweitert.  Zuerst wird der
    Klartext wie in Version 3 verschlüsselt.  Anschließend wird dieser v3‑Blob mit einem
    weiteren XOR‑Pad verwürfelt, das aus dem Master‑Passwort und einem separaten Salt
    abgeleitet wird.  Eine zusätzliche HMAC sichert diesen äußeren Container.  Für ältere
    Dateiformate (Version <4) entfällt die äußere Schicht, und der Triple‑Blob wird direkt
    zurückgegeben.
    """
    ...
    # Der finale Header (äußerer Container) verwendet die globale VERSION
    final_header = MAGIC + VERSION.to_bytes(1, "big") + kdf_tlv
    # Für die innere Triple‑Encryption wird Version 3 verwendet, um die Kompatibilität
    # zur bestehenden Entschlüsselungsroutine zu wahren, wenn Version >=4
    inner_version = 3 if VERSION >= 4 else VERSION
    inner_header = MAGIC + inner_version.to_bytes(1, "big") + kdf_tlv

    # Leite die drei Basisschlüssel aus dem Master‑Passwort und dem primären Salt ab
    ...
    # === Optionale äußere Schicht (Version >=4) ===
    if VERSION >= 4:
        # Generiere separates Salt für die äußere Schicht
        salt_extra = secrets.token_bytes(SALT_LEN)
        # Leite Schlüssel für das äußere Pad und HMAC direkt aus dem Master‑Passwort und salt_extra
        extra_pad_key = hmac_sha512(master_pw, salt_extra + b"extra_pad")
        extra_hmac_key = hmac_sha512(master_pw, salt_extra + b"extra_hmac")
        # Nonce für das äußere Pad
        nonce_extra = secrets.token_bytes(NONCE_LEN)
        # Erzeuge Pad und verschleiere den v3‑Blob
        extra_pad = pad_stream_from_mac(extra_pad_key, nonce_extra, len(triple_blob))
        final_cipher = xor_bytes(triple_blob, extra_pad)
        # HMAC über final_header||salt_extra||nonce_extra||final_cipher
        final_hmac = hmac_sha512(extra_hmac_key, final_header + salt_extra + nonce_extra + final_cipher)
        # Zusammensetzen des finalen Containers
        out = final_header + salt_extra + nonce_extra + final_cipher + final_hmac
        try:
            del salt_extra, extra_pad, final_cipher, extra_pad_key, extra_hmac_key, nonce_extra
        except Exception:
            pass
    else:
        # Für Version <4 wird der Triple‑Blob direkt verwendet
        out = triple_blob
...
def decrypt_vault_bytes(blob: bytes, master_pw: bytes) -> bytes:
    ...
    Diese Funktion unterstützt die Dateiformate Version 2, 3 und 4:

    * **Version 2** – Altes Format ohne KDF‑TLV und ohne Verwendung von Associated Data.  Die HMAC
      deckt nur den Datenkörper ab.
    * **Version 3** – Fügt ein KDF‑TLV hinzu und nutzt den Header als Associated Data für AES und
      ChaCha.  Die HMAC deckt Header und Datenkörper ab.
    * **Version 4** – Baut auf Version 3 auf, indem der komplette v3‑Blob mit einem weiteren
      One‑Time‑Pad und HMAC versehen wird.  Das zusätzliche Salt für diese Schicht wird
      separat gespeichert.
    """
    ...
    if version == 4:
        # ===== Entschlüsselung für Version 4 =====
        # Lese den KDF‑TLV wie in Version 3
        kdf_params, off = _parse_kdf_tlv(blob, off)
        # Der Header umfasst MAGIC + Version + TLV
        header = blob[:off]
        # Lese das zusätzliche Salt und Nonce für die äußere Schicht
        salt_extra = blob[off:off+SALT_LEN]; off += SALT_LEN
        nonce_extra = blob[off:off+NONCE_LEN]; off += NONCE_LEN
        # Der restliche Inhalt besteht aus dem verschleierten v3‑Blob und der HMAC
        if len(blob) < off + 64:
            raise ValueError("HMAC fehlt/Datei beschädigt")
        final_cipher = blob[off:-64]
        final_hmac = blob[-64:]
        # Leite die Schlüssel für die äußere Schicht aus dem Master‑Passwort und salt_extra ab
        extra_pad_key = hmac_sha512(master_pw, salt_extra + b"extra_pad")
        extra_hmac_key = hmac_sha512(master_pw, salt_extra + b"extra_hmac")
        # Berechne die erwartete HMAC und vergleiche
        calc_final = hmac_sha512(extra_hmac_key, header + salt_extra + nonce_extra + final_cipher)
        if not hmac.compare_digest(calc_final, final_hmac):
            raise ValueError("HMAC-Überprüfung (äußere Schicht) fehlgeschlagen — falsches Passwort oder manipulierte Datei")
        # Entferne das XOR‑Pad
        extra_pad = pad_stream_from_mac(extra_pad_key, nonce_extra, len(final_cipher))
        triple_blob = xor_bytes(final_cipher, extra_pad)
        # Entschlüssle den inneren v3‑Blob rekursiv
        try:
            plaintext = decrypt_vault_bytes(triple_blob, master_pw)
        finally:
            try:
                del extra_pad_key, extra_hmac_key, extra_pad, salt_extra, nonce_extra, final_cipher, final_hmac, calc_final
            except Exception:
                pass
        return plaintext
...
def cli_loop(path: Path, safe_mode: bool = SAFE_CLI_DEFAULT) -> None:
    ...
    if path.exists():
        ...
        print(tr(f"Treordatei nicht gefunden. Neuer Tresor wird erstellt: {path}",
    ...
    class App:
        ...
        def gui_create(self):
            ...
            if len(pw1) < MIN_MASTER_PW_LEN:
                if not messagebox.askyesno(
                    tr("Kurzes Passwort", "Short password"),
                    tr(
                        f"Passwort kürzer als {MIN_MASTER_PW_LEN}. Fortfahren?",
                        f"Password shorter than {MIN_MASTER_PW_LEN}. Continue?",
                    ),
                    parent=self.root,
                ):
                    return
        ...
        def build_main_ui(self):
            ...
            self.tree.heading("id", text=tr("ID", "ID"))
            self.tree.heading("label", text=tr("Label:", "Label:"))
            self.tree.heading("user", text=tr("Benutzer:", "User:"))
            self.tree.heading("email", text=tr("Email:", "Email:"))
        ...
        def gui_add(self):
            ...
            ttk.Label(frm, text=tr("Info:", "Info:")).grid(row=5, column=0, sticky="nw", pady=2)
            info_frame = ttk.Frame(frm)
            ...
            ttk.Checkbutton(frm, text=tr("Zeilenumbruch", "Word wrap"), variable=wrap_var, command=toggle_wrap).grid(row=6, column=1, sticky="w", pady=(2,0))
            # Button-Leiste
            btnf = ttk.Frame(frm)
            btnf.grid(row=7, column=1, sticky="w", pady=8)
            ttk.Button(btnf, text=tr("Hinzufügen", "Add"), command=on_add_click).pack(side="left", padx=4)
        ...
        def gui_open_file_ops_dialog(self):
            ...
            ttk.Button(
                ...
                command=gui_create_cover_image_generic,
            ).grid(row=0, column=2, sticky="w", padx=(12, 0))
            ttk.Button(
                hide_ops,
                text=tr("Bild aufblasen…", "Enlarge image…"),
                command=gui_inflate_image_generic,
