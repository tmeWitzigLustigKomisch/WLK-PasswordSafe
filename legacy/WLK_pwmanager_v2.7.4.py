...
def launch_gui(path: Path) -> None:
    ...
    class App:
        ...
        def toggle_dark_mode(self) -> None:
            """
            Schaltet zwischen hellem und dunklem Farbschema um.

            Beim Aktivieren des dunklen Modus werden globale Farbvariablen
            (``GUI_BG_COLOR``, ``GUI_FG_COLOR`` und ``GUI_BUTTON_COLOR``) auf
            vordefinierte Werte gesetzt. Beim Deaktivieren werden die zuvor
            gespeicherten Originalfarben wiederhergestellt.  Anschließend wird
            die aktuell sichtbare Oberfläche neu aufgebaut, damit die
            Farbänderungen sofort sichtbar werden.
            """
            # Greife auf die globalen Farbvariablen zu, um sie zu ändern
            ...
            try:
                if not getattr(self, "dark_mode", False):
                    ...
                    # Passe auch Eingabe‑ und Tabellenhintergründe an, damit sie im
                    # Dunkelmodus nicht als helle Blöcke hervorstechen.
                    ...
                    # Setze Eingabe‑ und Tabellenhintergrund zurück auf die
                    # Standardwerte
                    ENTRY_BG_COLOR = "#f5f5f5"
                    TABLE_BG_COLOR = "#f9f9f9"
            except Exception:
                pass
            # Versuche, das neue Farbschema auf das Tkinter‑Fenster und die Styles
            # anzuwenden. Dies entspricht der Logik im Konstruktor, wird hier
            # jedoch zur Laufzeit ausgeführt. Dadurch ändert sich der Hintergrund
            # sowie die Farbe von Rahmen, Labels und Buttons unmittelbar.
            try:
                import tkinter as tk
                from tkinter import ttk
                # Hintergrund des Fensters setzen.  Leerer String stellt den
                ...
                style = ttk.Style()
                # Wähle abhängig vom aktuellen Modus das passende Theme.  Im
                # Dunkelmodus nutzen wir "clam", weil es sich gut anpassen lässt.
                # Beim Zurückschalten in den Hellmodus kehren wir zum
                # ursprünglichen Theme zurück (falls verfügbar).  Dies verhindert
                # Darstellungsfehler wie in den Screenshots.
                try:
                    if getattr(self, 'dark_mode', False):
                        # Wir sind gerade in den Dunkelmodus gewechselt
                        style.theme_use('clam')
                    else:
                        # Wir wechseln zurück in den Hellmodus
                        orig = getattr(self, '_orig_theme', '')
                        if orig:
                            style.theme_use(orig)
                        else:
                            # Fallback: nutze ein Standard‑Theme
                            try:
                ...
                if GUI_BG_COLOR:
                    ...
                    style.configure('TLabel', background=GUI_BG_COLOR)
                    style.configure('TLabelframe', background=GUI_BG_COLOR)
                    # Hintergrund des Labels der Labelframe
                    style.configure('TLabelframe.Label', background=GUI_BG_COLOR, foreground=GUI_FG_COLOR or None)
                else:
                    # Entferne explizite Farben, um das Standard-Theme zu verwenden
                ...
                # Aktive/gedrückte Zustände für Buttons definieren.  Ohne diese
                # Zuweisungen würde der Hintergrund im Dunkelmodus komplett
                # weiß erscheinen, was unlesbar wäre.  Wähle je nach Modus
                ...
                try:
                    ...
                    pass
                # Eingabefelder und Comboboxen: nutze dynamische Hintergrundfarbe
                try:
                    style.configure('TEntry', fieldbackground=ENTRY_BG_COLOR, background=ENTRY_BG_COLOR)
                    style.configure('TCombobox', fieldbackground=ENTRY_BG_COLOR, background=ENTRY_BG_COLOR)
                except Exception:
                    pass
                # Treeview‑Stil: passe Hintergrund und Schrift an das aktuelle
                # Farbschema an.  Dies wirkt sich auf alle Treeviews aus, die
                # keinen eigenen Custom‑Stil setzen.
                try:
                    style.configure('Treeview', background=TABLE_BG_COLOR, fieldbackground=TABLE_BG_COLOR, foreground=GUI_FG_COLOR or None)
                    style.configure('Treeview.Heading', background=GUI_BG_COLOR or '', foreground=GUI_FG_COLOR or None)
                    # Hervorhebung bei Auswahl: dunklerer Button im Dunkelmodus, sonst hellgrau
                    sel_bg = GUI_BUTTON_COLOR if GUI_BUTTON_COLOR else '#ececec'
                    sel_fg = GUI_FG_COLOR if GUI_FG_COLOR else '#000000'
                    style.map('Treeview', background=[('selected', sel_bg)], foreground=[('selected', sel_fg)])
                except Exception:
                    pass
            except Exception:
                pass
            # Oberfläche neu zeichnen: je nach Kontext entweder Hauptansicht oder Login
            try:
                if getattr(self, "vault", None):
                    self.build_main_ui()
                else:
                    self.build_login_ui()
            except Exception:
                # Im Fehlerfall nichts tun; die Farben werden beim nächsten Wechsel angewandt
                pass
        ...
        def build_main_ui(self):
            ...
            # wird je nach Sprache angepasst.
        ...
        def gui_view(self):
            ...
            if is_table:
                ...
                # Zeilen einfügen; falls zu wenige/zu viele Werte, anpassen
        ...
        def gui_add(self):
            ...
            def create_table():
                ...
                for h in headers:
                    ...
                    tv.column(h, width=100, anchor="w")
                for r in range(rows):
                    tv.insert("", "end", values=["" for _ in headers])
        ...
        def gui_edit(self):
            ...
            def create_table():
                ...
                for r2 in range(rows):
                    tv2.insert("", "end", values=["" for _ in headers_local])
