...
# -*- coding: utf-8 -*-
"""
pwmanager.py — High-Security Einzeldatei Passwort-Manager (modular, deutsch kommentiert)

Kurz:
- Ein Master-Passwort (einzugeben) schützt eine portable Tresor-Datei.
- Triple-Layer-Verschlüsselung: AES-256-GCM, XOR-Obfuskation (HMAC-pad), ChaCha20-Poly1305.
- scrypt-KDF → drei unabhängige Schlüssel (AES_key, ChaCha_key, MAC_key).
- HMAC-SHA512 Integritätsschutz.
- Re-Randomizing: bei jedem Speichern neue Salt/Nonces/Pad — Datei ändert sich binär immer.
- Atomic save + Backup vor Überschreiben.
- GUI (Tkinter) und CLI, Export TXT/CSV, Clipboard mit Auto-Clear.
- Konfigurierbar: AUTOLOCK_MINUTES, scrypt-Parameter, sichere CLI-Option.
- Passwortstärken-Prüfung (on/off).
- Vollständig in einer Datei; kommentiert auf Deutsch.

Speichern als: pwmanager.py
Ausführen (GUI empfohlen):
    python pwmanager.py
CLI:
...
BACKUP_KEEP = 5                         # Anzahl Backup-Dateien (älteste löschen)
BACKUPS_ENABLED = False                  # Globale Option: Backups erstellen? True/False
...
def write_audit(action: str, details: str) -> None:
    """
    Schreibt einen Audit-Eintrag in die Logdatei, sofern Audit aktiviert ist.
    Jeder Eintrag besteht aus einem UNIX-Zeitstempel, einer Aktionsbezeichnung
    und optionalen Details (z. B. Eintrags-ID und Label). Aktionen können
    beispielsweise 'create', 'update', 'delete', 'export_entry', 'export_all'
    oder 'generate_password' sein.
    ...
    try:
        ts = time.time()
        # Öffne Audit-Log im Anhängemodus. Wenn die Datei noch nicht existiert,
        # wird sie mit restriktiven Rechten erstellt (POSIX: 0600). Nach dem
        # Öffnen wird die Berechtigung gesetzt, um sicherzustellen, dass auch
        # bestehende Dateien korrekt geschützt sind.
        ...
        # Fehler beim Schreiben des Logs sollten den Programmlauf nicht
        # beeinflussen. Fehler werden still ignoriert.
        pass
...
def encrypt_vault_bytes(plaintext: bytes, master_pw: bytes) -> bytes:
    ...
    ciphertext_chacha = chacha.encrypt(nonce_chacha, obf, None)

    file_body = salt + nonce_aes + nonce_pad + nonce_chacha + ciphertext_chacha
    # Integritätsprüfung mit separatem HMAC‑Key
    file_hmac = hmac_sha512(hmac_key, file_body)

    header = MAGIC + VERSION.to_bytes(1, "big")
...
def decrypt_vault_bytes(blob: bytes, master_pw: bytes) -> bytes:
    ...
    min_len = 4 + 1 + SALT_LEN + NONCE_LEN * 3 + 64
    if len(blob) < min_len:
    ...
    aes_key, chacha_key, mac_key = derive_three_keys(master_pw, salt)
    # Leite separate Schlüssel ab wie bei encrypt_vault_bytes
    ...
    # HMAC prüfen mit hmac_key
    calc_hmac = hmac_sha512(hmac_key, file_body)
    if not hmac.compare_digest(calc_hmac, file_hmac):
        # wipe keys before raising
        try:
            del aes_key, chacha_key, pad_key, hmac_key, calc_hmac
        except Exception:
            pass
    ...
    try:
        ...
        # wipe keys
        try:
            del aes_key, chacha_key, pad_key, hmac_key
        except Exception:
            pass
        raise ValueError("ChaCha-Entschlüsselung fehlgeschlagen") from e

    # Pad wiederherstellen & deobf
    ...
    try:
        ...
        # wipe keys
        try:
            del aes_key, chacha_key, pad_key, hmac_key, pad, obf, ciphertext_aes
        except Exception:
            pass
        raise ValueError("AES-Entschlüsselung fehlgeschlagen (falsches Passwort/korrupt)") from e

    # versuchen sensitive zu löschen
    try:
        del aes_key, chacha_key, pad_key, hmac_key, calc_hmac, pad, obf, ciphertext_aes
    except Exception:
        pass
...
# ====================================
# SECTION E2 — Beliebige Dateien verschlüsseln & verstecken
# ====================================
# Neben der Tresor-Verwaltung ermöglicht der Passwortmanager das
# Verschlüsseln beliebiger Dateien und das Verstecken von Dateien
# in einer anderen Datei. Hierfür wird derselbe Triple-Layer-Algorithmus
# verwendet wie für den Tresor (AES‑GCM, XOR‑Pad, ChaCha20‑Poly1305).
# Das versteckte Format fügt eine Markierung am Ende der Datei an,
# gefolgt von der Länge der verschlüsselten Nutzlast und der Nutzlast selbst.
# Beim Entschlüsseln wird die Markierung erkannt, die Länge gelesen
# und anschließend der verschlüsselte Teil mit dem Passwort entschlüsselt.

# Kennzeichnung am Dateiende für versteckte Dateien. Wird genutzt, um die
# verschlüsselte Nutzlast beim Extrahieren zu finden. Der Marker besteht
# aus vier Bytes, um Kollisionen mit Zufallsdaten zu minimieren.
STEGO_MARKER = b"PWSF"  # Password‑manager Stego‑File Marker
STEGO_LENGTH_LEN = 8      # Anzahl der Bytes zur Speicherung der Länge (64‑Bit)
...
def export_entry_txt(v: Vault, eid: str, outpath: Optional[Path] = None) -> Path:
    ...
    content = textwrap.dedent(f"""\
        ...
        Erstellt    : {time.ctime(e.created_at)}
        Geändert    : {time.ctime(e.updated_at)}
    """)
    ...
    with open(fname, "w", newline="", encoding="utf-8") as f:
        ...
        for e in v.entries.values():
            writer.writerow([e.id, e.label, e.username, e.email, e.password, e.info, e.website,
                             time.ctime(e.created_at), time.ctime(e.updated_at)])
    return fname
...
def launch_gui(path: Path) -> None:
    ...
    class App:
        ...
        def build_login_ui(self):
            ...
            ttk.Label(frm, text="Master-Passwort", font=("TkDefaultFont", 14)).pack(pady=(10, 6))
            ...
            ttk.Button(btns, text="Öffnen", command=self.gui_unlock).pack(side="left", padx=6)
            ttk.Button(btns, text="Neuer Tresor", command=self.gui_create).pack(side="left", padx=6)
            ttk.Button(btns, text="Tresor-Datei wählen", command=self.gui_select_file).pack(side="left", padx=6)
            # Konfig laden: ermöglicht Auswahl einer Konfigurationsdatei
            ttk.Button(btns, text="Konfig laden", command=self.gui_select_config).pack(side="left", padx=6)
            ttk.Button(btns, text="Konfig erstellen", command=self.gui_create_config).pack(side="left", padx=6)
            # Zusätzliche Schaltfläche, um die geladene Konfiguration direkt im Programm zu bearbeiten
            ttk.Button(btns, text="Konfig bearbeiten", command=self.gui_edit_config).pack(side="left", padx=6)
            ttk.Button(btns, text="Hilfe", command=self.show_help).pack(side="left", padx=6)
            # Berechne Tresor- und Konfigurationsstatus zur Anzeige
            try:
                def_vault = default_vault_path()
            except Exception:
                def_vault = None
            # Tresorstatus (standard vs. extern) und Farbcodierung
            if def_vault and Path(self.path).resolve() == def_vault.resolve():
                if self.path.exists():
                    vault_msg = f"Standard-Tresor-Datei: {self.path} (vorhanden)"
                else:
                    vault_msg = f"Standard-Tresor-Datei: {self.path} (wird bei Bedarf angelegt)"
            else:
                if self.path.exists():
                    vault_msg = f"Externe Tresor-Datei: {self.path}"
                else:
                    vault_msg = f"Externe Tresor-Datei: {self.path} (wird bei Bedarf angelegt)"
            vault_color = "green" if self.path.exists() else "red"
            ...
            # Statusausgabe
            ttk.Label(frm, text="Status:", foreground="blue").pack(anchor="w")
            ...
            file_ops = ttk.LabelFrame(frm, text="Datei-Operationen", padding=8)
            file_ops.pack(fill="x", pady=(8, 8))
            ttk.Label(file_ops,
                      text=("Hier können Sie beliebige Dateien verschlüsseln, entschlüsseln oder in andere Dateien verstecken.\n"
            ...
            steg_frame = ttk.LabelFrame(file_ops, text="Datei verstecken und extrahieren", padding=6)
            steg_frame.pack(fill="x", pady=(6, 0))
            ttk.Label(steg_frame,
                      text=("Datei verstecken: Wählen Sie zunächst die zu versteckende Datei, dann die Cover-Datei (Träger) und"
                            " anschließend einen Ausgabepfad.\nDer Inhalt wird verschlüsselt und ans Ende der Cover-Datei angehängt."),
                      wraplength=700,
                      justify="left").pack(anchor="w", pady=(0, 4))
            hide_ops = ttk.Frame(steg_frame)
            hide_ops.pack(fill="x", pady=(0, 8))
            ttk.Button(hide_ops, text="Zu versteckende Datei", command=self.gui_select_hide_data).grid(row=0, column=0, sticky="w", pady=2)
            ttk.Label(hide_ops, textvariable=self.hide_data_path, wraplength=500).grid(row=0, column=1, sticky="w", padx=6)
            ttk.Button(hide_ops, text="Cover-Datei", command=self.gui_select_hide_cover).grid(row=1, column=0, sticky="w", pady=2)
            ttk.Label(hide_ops, textvariable=self.hide_cover_path, wraplength=500).grid(row=1, column=1, sticky="w", padx=6)
            ttk.Button(hide_ops, text="Ziel (.hid)", command=self.gui_select_hide_output).grid(row=2, column=0, sticky="w", pady=2)
            ttk.Label(hide_ops, textvariable=self.hide_output_path, wraplength=500).grid(row=2, column=1, sticky="w", padx=6)
            ttk.Button(hide_ops, text="Verstecken", command=self.gui_do_hide).grid(row=3, column=0, sticky="w", pady=(4, 6))
            ttk.Label(steg_frame,
                      text=("Verstecktes extrahieren: Wählen Sie die .hid-Datei mit verstecktem Inhalt und anschließend einen\n"
            ...
            ttk.Button(extract_ops, text=".hid-Datei", command=self.gui_select_extract_stego).grid(row=0, column=0, sticky="w", pady=2)
            ttk.Label(extract_ops, textvariable=self.extract_stego_path, wraplength=500).grid(row=0, column=1, sticky="w", padx=6)
            ttk.Button(extract_ops, text="Ziel-Datei", command=self.gui_select_extract_output).grid(row=1, column=0, sticky="w", pady=2)
        ...
        def build_main_ui(self):
            ...
            try:
                ...
                def_vault = None
            # Formuliere Statusmeldung zum Tresor
            if def_vault and Path(self.path).resolve() == def_vault.resolve():
                if self.path.exists():
                    vault_msg = f"Standard-Tresor-Datei: {self.path} (vorhanden)"
                else:
            ...
            ttk.Button(buttons_frame, text="Lock (verschlüsseln und schließen)", command=self.lock).pack(side="left", padx=4)
            ttk.Button(buttons_frame, text="Hilfe", command=self.show_help).pack(side="left", padx=4)
            ttk.Button(buttons_frame, text="Konfig erstellen", command=self.gui_create_config).pack(side="left", padx=4)
            ttk.Button(buttons_frame, text="Konfig laden", command=self.gui_select_config).pack(side="left", padx=4)
            # Schaltfläche zum Bearbeiten der aktuellen Konfiguration
            ttk.Button(buttons_frame, text="Konfig bearbeiten", command=self.gui_edit_config).pack(side="left", padx=4)
            ...
            ttk.Button(side, text="Anzeigen", command=self.gui_view).pack(fill="x", pady=3)
            ttk.Button(side, text="Hinzufügen", command=self.gui_add).pack(fill="x", pady=3)
            ttk.Button(side, text="Ändern", command=self.gui_edit).pack(fill="x", pady=3)
            ttk.Button(side, text="Löschen", command=self.gui_delete).pack(fill="x", pady=3)
            ttk.Button(side, text="Export (Entry .txt)", command=self.gui_export_entry).pack(fill="x", pady=3)
            ttk.Button(side, text="Export (Alle .txt)", command=self.gui_export_all).pack(fill="x", pady=3)
            # CSV‑Export und Import zusammen, damit alle Datei‑Im-/Export‑Funktionen gruppiert sind.
            ttk.Button(side, text="Export CSV", command=self.gui_export_csv).pack(fill="x", pady=3)
            ttk.Button(side, text="Import CSV", command=self.gui_import_csv).pack(fill="x", pady=3)
            ttk.Button(side, text="Generiere Passwort", command=self.gui_gen_pw).pack(fill="x", pady=3)
            ttk.Button(side, text="Master-PW ändern", command=self.gui_change_master_pw).pack(fill="x", pady=3)
            ttk.Button(side, text="Neu verschlüsseln (save)", command=self.gui_resave).pack(fill="x", pady=6)
            ...
            ttk.Button(side, text="Datei-Operationen", command=self.gui_open_file_ops_dialog).pack(fill="x", pady=6)
        ...
        def gui_view(self):
            ...
            Die Ansicht verwendet ein Grid‑Layout mit scrollbarem Textfeld, das sich an
            die Fenstergröße anpasst. Zusätzlich kann der Benutzer den Zeilenumbruch
            ein- oder ausschalten. Das Passwort kann aus dem Dialog heraus in die
            ...
            if not sel:
                return
            # Treeview liefert bei rein numerischen IDs ints; immer zu str konvertieren.
            iid = str(self.tree.item(sel[0])["values"][0])
            e = self.vault.entries.get(iid)
            if not e:
                return
            # Neues Fenster für die Detailansicht
            top = tk.Toplevel(self.root)
            top.title(f"Anzeigen: {e.label}")
            ...
            content = (
                f"Label       : {e.label}\n"
                f"Benutzer    : {e.username}\n"
                f"Email       : {e.email}\n"
                f"Passwort    : {e.password}\n"
                f"Info        : {e.info}\n"
                f"Webseite/IP : {e.website}\n"
                f"Erstellt    : {time.ctime(e.created_at)}\n"
                f"Geändert    : {time.ctime(e.updated_at)}\n"
            )
            txt.insert("1.0", content)
            txt.config(state="disabled")
            # Scrollbars für vertikal und horizontal
            y_scroll = ttk.Scrollbar(text_frame, orient="vertical", command=txt.yview)
            x_scroll = ttk.Scrollbar(text_frame, orient="horizontal", command=txt.xview)
            ...
            def toggle_wrap():
                ...
                txt.config(wrap="word" if wrap_var.get() else "none")
                txt.config(state="disabled")
            ttk.Checkbutton(frm, text="Zeilenumbruch", variable=wrap_var, command=toggle_wrap).grid(
                row=1, column=0, sticky="w", pady=(4, 0)
            )
            # Buttons
            btnf = ttk.Frame(frm)
            btnf.grid(row=2, column=0, sticky="w", pady=8)
            ttk.Button(
                btnf,
                text="Kopiere Passwort",
                command=lambda pw=e.password: self.copy_pw_and_clear(pw),
            ).pack(side="left", padx=4)
            ttk.Button(btnf, text="Schließen", command=top.destroy).pack(side="left", padx=4)
        ...
        def gui_edit(self):
            ...
            btnf = ttk.Frame(frm)
            btnf.grid(row=7, column=1, sticky="w", pady=8)
            ttk.Button(btnf, text="Speichern", command=do_save).pack(side="left", padx=4)
            ttk.Button(btnf, text="Abbrechen", command=top.destroy).pack(side="left", padx=4)
