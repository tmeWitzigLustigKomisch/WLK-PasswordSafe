    ...
    class App:
        ...
        def gui_add(self):
            ...
            def edit_cell(event):
                ...
                if region != "cell":
                    return
                # Falls bereits ein Zellenbearbeitungsfenster geöffnet ist, hebe es hervor und öffne kein neues
                existing = getattr(table_tree, "_edit_cell_window", None)
                try:
                ...
                # Speichere das Bearbeitungsfenster, um Mehrfachöffnungen zu verhindern
                try:
                    table_tree._edit_cell_window = edit_win
                except Exception:
                    pass
                ...
                def commit():
                    ...
                    edit_win.destroy()
                    # Rücksetzen der Fensterreferenz, wenn das Fenster geschlossen wird
                    try:
                        table_tree._edit_cell_window = None
                    except Exception:
                        pass
                ttk.Button(edit_win, text=tr("OK", "OK"), command=commit).pack(pady=(0,6))
                # Enter löst die gleiche Aktion wie der OK-Button aus
        ...
        def gui_edit(self):
            ...
            def edit_cell(event):
                ...
                if region != "cell":
                    return
                # Falls bereits ein Zellenbearbeitungsfenster existiert, hebe es hervor und öffne kein neues
                existing2 = getattr(table_tree, "_edit_cell_window", None)
                try:
                    if existing2 is not None and existing2.winfo_exists():
                        existing2.lift()
                        return
                except Exception:
                    pass
