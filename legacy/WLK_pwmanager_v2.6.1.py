...
def launch_gui(path: Path) -> None:
    ...
    class App:
        def __init__(self, root, path: Path):
            ...
            if GUI_BG_COLOR or GUI_FG_COLOR or GUI_BUTTON_COLOR:
                try:
                    ...
                    # Setze helle Hintergründe für Eingabefelder, um Kontrast zu verbessern
                    try:
                        style.configure('TEntry', fieldbackground='#f5f5f5')
                        style.configure('TCombobox', fieldbackground='#f5f5f5')
                    except Exception:
                        pass
                except Exception:
        ...
        def gui_edit_config(self):
            ...
            # Ermögliche Scrollen mit dem Mausrad nur innerhalb des Canvas.
            try:
        ...
        def gui_add(self):
            ...
            # Buttons zum Anlegen einer Tabelle und zum Bearbeiten vorhandener Spalten
            btn_create_table = ttk.Button(frm, text=tr("Tabelle erstellen", "Create table"), command=create_table)
            # Ermöglicht es, vorhandene Tabellenspalten umzubenennen oder neue hinzuzufügen
            ...
            def on_format_change(event=None):
                ...
                for child in info_container.winfo_children():
                    child.grid_forget()
                # Verstecke Buttons standardmäßig
                btn_create_table.grid_remove()
                btn_edit_columns.grid_remove()
                if fmt == tr("Tabelle", "Table"):
                    # Tabelle anzeigen und Buttons einblenden
                    table_frame.grid(row=0, column=0, sticky="nsew")
                    btn_create_table.grid(row=7, column=1, sticky="w", pady=4)
                    btn_edit_columns.grid(row=7, column=2, sticky="w", pady=4)
                else:
