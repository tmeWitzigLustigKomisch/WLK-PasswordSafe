    ...
    class App:
        ...
        def gui_edit_config(self):
            ...
            try:
                ...
                pass
            # Erzeuge einen eigenen Stil für Entry-Felder in diesem Dialog, um einen helleren
            # Hintergrund zu erhalten. Dadurch sind Eingaben auf dunklen GUIs besser lesbar.
            try:
                cfg_style = ttk.Style()
                cfg_style.configure('Config.TEntry', fieldbackground='#f5f5f5', background='#f5f5f5')
            except Exception:
                pass
            ...
            for idx, key in enumerate(CONFIG_KEYS):
                ...
                # Verwende den hellen Entry-Stil für bessere Lesbarkeit
                try:
                    ent = ttk.Entry(scrollable_frame, width=40, style='Config.TEntry')
                except Exception:
        ...
        def gui_add(self):
            ...
            def on_format_change(event=None):
                ...
                if fmt == tr("Tabelle", "Table"):
                    # Tabelle anzeigen und Buttons einblenden
                    table_frame.grid(row=0, column=0, sticky="nsew")
                    table_btn_frame.grid(row=7, column=1, sticky="w", pady=4)
                else:
