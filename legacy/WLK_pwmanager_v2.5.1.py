...
# --- Helpers: robust boolean parsing & hardening enforcement ---
def parse_bool(v):
    ...
    if isinstance(v, (int, float)):
        return v != 0
    if isinstance(v, str):
        return v.strip().lower() in {"1","true","t","yes","y","ja","j","on"}
    return False

def _enforce_hardened_safe_mode():
    g = globals()
    if parse_bool(g.get("HARDENED_SAFE_MODE", False)):
        g["REQUIRE_EXPLICIT_EXPORT_CONFIRM"] = True
        for flag in ("SAFE_BLOCK_EXPORT","SAFE_BLOCK_CSV","SAFE_BLOCK_STEGO","SAFE_BLOCK_CLIPBOARD","NO_PLAINTEXT_IN_GUI"):
            if flag in g:
                g[flag] = True
        try:
            g["CLI_CLIPBOARD_CLEAR_SECONDS"] = max(10, int(g.get("CLI_CLIPBOARD_CLEAR_SECONDS", 30)))
        except Exception:
            g["CLI_CLIPBOARD_CLEAR_SECONDS"] = 30
...
# Dateien und höherem CPU‑Aufwand.
...
CONFIG_EXPLANATIONS: Dict[str, str] = {
    ...
    "EXTRA_ENCRYPTION_LAYERS": "Zusätzliche Verschlüsselungsschichten jenseits der Triple-Verschlüsselung. 0 = keine Zusatzschicht (nur Triple‑Layer), 1 = eine Schicht (Version 4), 2 = zwei Schichten (Version 5) usw. Es gibt kein festes Maximum – jede zusätzliche Schicht erhöht die Dateigröße und den Zeitaufwand.",
...
def apply_config(cfg: Dict[str, object]) -> None:
    ...
    for key, value in cfg.items():
        if key == "AUTOLOCK_MINUTES":
            ...
            BACKUPS_ENABLED = parse_bool(value)
        elif key == "SAFE_CLI_DEFAULT":
            SAFE_CLI_DEFAULT = parse_bool(value)
        elif key == "KDF_MODE":
            ...
            try:
                globals()["SHOW_TELEGRAM_AD"] = parse_bool(value)
            except Exception:
            ...
            try:
                ...
                layers = 0
            # Nicht-negative Ganzzahl; zu viele Schichten können zu übermäßig großen Dateien führen
            layers = max(0, layers)
            globals()["EXTRA_ENCRYPTION_LAYERS"] = layers
            # Aktualisiere das Dateiformat-Version entsprechend (Basis 3)
...
def derive_three_keys(master_pw: bytes, salt: bytes) -> Tuple[bytes, bytes, bytes]:
    ...
    if KDF_MODE == "argon2" and _HAS_ARGON2:
        ...
        if _CryptoScrypt is not None:
            ...
            # 
            # Fallback: hashlib.scrypt mit dynamischem maxmem.
            required = 128 * KDF_N * KDF_R * KDF_P
            avail = None
            try:
                # Best effort: Hälfte des verfügbaren RAM nutzen
                if hasattr(os, "sysconf"):
                    pagesize = os.sysconf("SC_PAGESIZE")
                    avphys = os.sysconf("SC_AVPHYS_PAGES")
                    avail = pagesize * avphys
            except Exception:
                avail = None
            if avail is None:
                # konservative Annahme: 1 GiB
                avail = 1 * 1024 * 1024 * 1024
            # mindestens required*2, höchstens Hälfte des verfügbaren RAM, oder required*4 – was kleiner ist
            maxmem = max(required * 2, min(avail // 2, required * 4))
            if maxmem < required * 2:
                raise RuntimeError(
                    "Scrypt-Fallback nicht sicher (zu wenig RAM verfügbar). Bitte 'cryptography' installieren "
                    "oder Argon2 verwenden."
                )
            dk = hashlib.scrypt(master_pw, salt=salt, n=KDF_N, r=KDF_R, p=KDF_P, maxmem=maxmem, dklen=KDF_DKLEN) "oder KDF‑Parameter in der Konfig reduzieren.")
            dk = hashlib.scrypt(
                password=master_pw,
                salt=salt,
                n=KDF_N,
    ...
    **Schichtaufbau:** Zunächst wird der Klartext mittels Triple‑Verschlüsselung verarbeitet
    ...
    if layers > 0:
        ...
        for i in range(layers):
            ...
            pad_key_i = hmac_sha512(master_pw, salt_extra + f"layer{i}_pad".encode())
            hmac_key_i = hmac_sha512(master_pw, salt_extra + f"layer{i}_hmac".encode())
    ...
    if version >= 4:
        ...
        try:
            ...
            for i in reversed(range(layers)):
                ...
                # Leite die Pad- und HMAC-Schlüssel aus master_pw, salt_i und dem Layer-Index ab
                pad_key_i = hmac_sha512(master_pw, salt_i + f"layer{i}_pad".encode())
                hmac_key_i = hmac_sha512(master_pw, salt_i + f"layer{i}_hmac".encode())
            ...
            if version == 4 and layers == 1:
                ...
                extra_pad_key = hmac_sha512(master_pw, salt_i + b"extra_pad")
                extra_hmac_key = hmac_sha512(master_pw, salt_i + b"extra_hmac")
...
try:
    def _wlk_guard_wrapper(fn):
        def _wrapped(self, *args, **kwargs):
            try:
                hm = parse_bool(HARDENED_SAFE_MODE)
            except Exception:
                hm = False
            if hm:
                try:
